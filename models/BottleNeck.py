...
class MDCR(nn.Module):
    def __init__(self, in_features, out_features, norm_type='bn', activation=True, rate=[2, 8, 12, 18]):
        super().__init__()
...
if __name__ == '__main__':
    # 创建一个随机输入张量作为示例
    input_tensor = torch.randn(1, 512, 8, 8)  # 假设输入尺寸为 [batch_size, channels, height, width]

    # 实例化 MDCR 模块
    mdcr = MDCR(in_features=512, out_features=512)
